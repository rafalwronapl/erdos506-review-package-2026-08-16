...
/-!
# The seventeen-point selected-eight-circle endpoint

The defect-corrected selected-circle functional forces weight at least
`241`, while elementary two--two capacity gives at most `144`.
-/
...
private theorem seventeenEight_relative_rows
    (S : BlockSystem Point Block) (D : Finset Point)
    (hpoint : Fintype.card Point = 17) (hD : D.card = 8) :
    (∑ b : Block, (Nat.choose (fourteenOutside S D b) 3 : Int)) = 84 ∧
    (∑ b : Block,
      (fourteenInside S D b : Int) *
        (Nat.choose (fourteenOutside S D b) 2 : Int)) = 288 ∧
    (∑ b : Block,
      (Nat.choose (fourteenInside S D b) 2 : Int) *
        (fourteenOutside S D b : Int)) = 252 ∧
...
private theorem seventeenEight_line_residual_bound (g x : Nat)
...
private theorem seventeenEight_circle_residual_bound (g x : Nat)
    (hg : g ≤ 2) (hmin : 3 ≤ g + x) (hcap : g + x ≤ 8) :
...
private theorem seventeenEight_selected_circle_residual :
...
private theorem sum_seventeenEightBlockFunctional_le
    (S : BlockSystem Point Block) (D : Finset Point) (gamma : Block)
    (hpoint : Fintype.card Point = 17) (hD : D.card = 8)
    (hgammaKind : S.kind gamma = .circle)
    (hgammaSupport : S.support gamma = D)
    (hcircleCap : ∀ b, S.kind b = .circle → (S.support b).card ≤ 8)
    (hlineCap : ∀ b, S.kind b = .line → (S.support b).card ≤ 8) :
    (∑ b : Block,
      fourteenBlockFunctional (S.kind b)
        (fourteenInside S D b) (fourteenOutside S D b)) ≤
      4 * S.defectRow + 328 := by
  classical
  have hrow0 := (seventeenEight_relative_rows S D hpoint hD).1
  have hlocal (b : Block) :
      fourteenBlockFunctional (S.kind b)
          (fourteenInside S D b) (fourteenOutside S D b) ≤
    ...
    · subst b
      ...
      rw [seventeenEight_selected_circle_residual, hdefectGamma]
      norm_num [Nat.choose]
    · have hg : fourteenInside S D b ≤ 2 := by
      ...
      | circle =>
          ...
          have hres := seventeenEight_circle_residual_bound
            (fourteenInside S D b) (fourteenOutside S D b)
            hg hmin hcap
          have hdefect' :
      ...
      | line =>
          ...
          by_cases hmin :
              3 ≤ fourteenInside S D b + fourteenOutside S D b
          · have hres := seventeenEight_line_residual_bound
              (fourteenInside S D b) (fourteenOutside S D b)
              hg hmin hcap
            have hdefect' :
  ...
  calc
    ...
    _ = 4 * S.defectRow + 328 := by
      simp only [Finset.sum_add_distrib, ← Finset.mul_sum]
      rw [S.defectRow_eq_sum_blockDefectContribution, hrow0]
      simp
      ring

private theorem no_selected_eight_block_at_seventeen
    (S : BlockSystem Point Block) (D : Finset Point) (gamma : Block)
    (hpoint : Fintype.card Point = 17) (hD : D.card = 8)
    (hgammaKind : S.kind gamma = .circle)
    (hgammaSupport : S.support gamma = D)
    ...
    (hdefect : S.defectRow ≤ 221)
    (hcircles : S.totalCircleCount ≤ 112) : False := by
  have hexpand := sum_seventeenEightBlockFunctional_eq S D hpoint hD
  have hupper := sum_seventeenEightBlockFunctional_le S D gamma hpoint hD
    hgammaKind hgammaSupport hcircleCap hlineCap
  have hupper' :
      (∑ b : Block,
        fourteenBlockFunctional (S.kind b)
          (fourteenInside S D b) (fourteenOutside S D b)) ≤ 1212 := by
    omega
  have hresidual :
      (2658 : Int) - 6 * (fourteenWeight S D : Int) ≤ 1212 := by
    rw [hexpand] at hupper'
    omega
  have hlower : 241 ≤ fourteenWeight S D := by omega
...
/-- The `(17,8)` circle-kind finite-window residual is impossible. -/
theorem FiniteWindowRichBlockResidual.circle_impossible_of_seventeen_eight
  ...
  have hcapEight : BlockSizeCap S 8 := by
    ...
    norm_num at hcap
    exact hcap
  have hkindCap : ∀ b, 3 ≤ (S.support b).card →
      (S.support b).card ≤ 8 := hcapEight
  have hcircleCap : ∀ b, S.kind b = .circle → (S.support b).card ≤ 8 := by
    intro b hb
    exact hkindCap b (S.circle_min b hb)
  have hlineCap : ∀ b, S.kind b = .line → (S.support b).card ≤ 8 := by
    intro b hb
    by_cases hthree : 3 ≤ (S.support b).card
    · exact hkindCap b hthree
  ...
  have hmomentX : 27 ≤ S.subsetPivotMoment (Finset.univ \ D) := by
    ...
    norm_num at hmoment ⊢
    exact hmoment
  have hdefect : S.defectRow ≤ 221 := by
    ...
    norm_num at hrow ⊢
    exact hrow
  have hcount112 : Erdos506.V4.circleCount cfg ≤ 112 := by
    rw [h17] at hcount
  ...
  have htotal : S.totalCircleCount ≤ 112 := by
    ...
    exact hcount112
  apply no_selected_eight_block_at_seventeen S D gamma h17 hDcard
