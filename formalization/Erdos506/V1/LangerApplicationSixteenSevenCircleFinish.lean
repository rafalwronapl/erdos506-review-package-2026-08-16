...
/-!
# The sixteen-point selected-seven-circle endpoint

The completed selected-eight endpoint lowers the circle cap from eight to
seven.  A selected-circle residual row then forces an outside-pair weight
of at least `150`, while the elementary two--two capacity bounds it by
`108`.
-/
...
/-- The four relative triple rows for the selected `7+9` split. -/
private theorem sixteenSeven_relative_rows
    (S : BlockSystem Point Block) (D : Finset Point)
    (hpoint : Fintype.card Point = 16) (hD : D.card = 7) :
    (∑ b : Block, (Nat.choose (fourteenOutside S D b) 3 : Int)) = 84 ∧
    (∑ b : Block,
      (fourteenInside S D b : Int) *
        (Nat.choose (fourteenOutside S D b) 2 : Int)) = 252 ∧
    (∑ b : Block,
      (Nat.choose (fourteenInside S D b) 2 : Int) *
        (fourteenOutside S D b : Int)) = 189 ∧
    (∑ b : Block, (Nat.choose (fourteenInside S D b) 3 : Int)) = 35 := by
...
/-- Pointwise residual summation after the eight-circle branch is removed. -/
private theorem sum_sixteenSevenBlockFunctional_le
    ...
    (hlineCap : ∀ b, S.kind b = .line → (S.support b).card ≤ 8) :
    (∑ b : Block,
      fourteenBlockFunctional (S.kind b)
        (fourteenInside S D b) (fourteenOutside S D b)) ≤
      4 * S.defectRow + 279 := by
  classical
  have hrow0 := (sixteenSeven_relative_rows S D hpoint hD).1
  have hlocal (b : Block) :
      fourteenBlockFunctional (S.kind b)
          (fourteenInside S D b) (fourteenOutside S D b) ≤
    ...
    · have hg : fourteenInside S D b ≤ 2 := by
      ...
      | line =>
          ...
          have hcap :
              fourteenInside S D b + fourteenOutside S D b ≤ 8 := by
            rw [hsum]
            exact hlineCap b hkind
          by_cases hmin :
              3 ≤ fourteenInside S D b + fourteenOutside S D b
          · have hres := sixteenSeven_line_residual_bound
              (fourteenInside S D b) (fourteenOutside S D b)
              hg hmin hcap
            have hdefect' :
  ...
  calc
    ...
    _ = 4 * S.defectRow + 279 := by
      simp only [Finset.sum_add_distrib, ← Finset.mul_sum]
      rw [S.defectRow_eq_sum_blockDefectContribution, hrow0]
      simp
      ring

/-- The selected `7+9` row contradicts elementary pair capacity. -/
private theorem no_selected_seven_block_at_sixteen
    (S : BlockSystem Point Block) (D : Finset Point) (gamma : Block)
    (hpoint : Fintype.card Point = 16) (hD : D.card = 7)
    (hgammaKind : S.kind gamma = .circle)
    (hgammaSupport : S.support gamma = D)
    ...
    (hdefect : S.defectRow ≤ 192)
    (hcircles : S.totalCircleCount ≤ 98) : False := by
  have hexpand := sum_sixteenSevenBlockFunctional_eq S D hpoint hD
  have hupper := sum_sixteenSevenBlockFunctional_le S D gamma hpoint hD
    hgammaKind hgammaSupport hcircleCap hlineCap
  have hupper' :
      (∑ b : Block,
        fourteenBlockFunctional (S.kind b)
          (fourteenInside S D b) (fourteenOutside S D b)) ≤ 1047 := by
    omega
  have hresidual :
      (1947 : Int) - 6 * (fourteenWeight S D : Int) ≤ 1047 := by
    rw [hexpand] at hupper'
    omega
  have hlower : 150 ≤ fourteenWeight S D := by omega
...
/-- The `(16,7)` circle-kind finite-window residual is impossible. -/
theorem FiniteWindowRichBlockResidual.circle_impossible_of_sixteen_seven
  ...
  have hcapEight : BlockSizeCap S 8 := by
    ...
    norm_num at hcap
    exact hcap
  have hcircleCap : ∀ b, S.kind b = .circle → (S.support b).card ≤ 7 := by
    intro b hb
    have hle : (S.support b).card ≤ 8 :=
      hcapEight b (S.circle_min b hb)
    by_contra hnot
    have height : (S.support b).card = 8 := by omega
    have height' : (geometricBlockSupport cfg b).card = 8 := by
      simpa [S] using height
    have hb' : (blockSystem cfg).kind b = .circle := by
      simpa [S] using hb
    let R8 : FiniteWindowRichBlockResidual cfg :=
      { window_lower := by rw [h16]; omega
        window_upper := by rw [h16]; omega
        block := b
        nontrivial := by omega
        aboveThreshold := by
          rw [h16, height']
    ...
    exact (R8.circle_impossible_of_sixteen_eight
      Mel EvenArr hadm hb' h16 height' hcount).elim
  have hlineCap : ∀ b, S.kind b = .line → (S.support b).card ≤ 8 := by
    intro b hb
    by_cases hthree : 3 ≤ (S.support b).card
    · exact hcapEight b hthree
  ...
  have hDcard : D.card = 7 := by simpa [D, S] using hseven
  have hXcard : (Finset.univ \ D).card = 9 := by
    rw [Finset.card_sdiff_of_subset (Finset.subset_univ D),
      Finset.card_univ, h16, hDcard]
  have hmomentD : 21 ≤ S.subsetPivotMoment D := by
  ...
  have hmomentX : 27 ≤ S.subsetPivotMoment (Finset.univ \ D) := by
    ...
    norm_num at hmoment ⊢
    exact hmoment
  have hdefect : S.defectRow ≤ 192 := by
    ...
    norm_num at hrow ⊢
    exact hrow
  have hcount98 : Erdos506.V4.circleCount cfg ≤ 98 := by
    rw [h16] at hcount
  ...
  have htotal : S.totalCircleCount ≤ 98 := by
    ...
    exact hcount98
  apply no_selected_seven_block_at_sixteen S D gamma h16 hDcard
