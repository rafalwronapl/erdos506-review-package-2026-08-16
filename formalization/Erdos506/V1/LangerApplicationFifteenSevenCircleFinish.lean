...
/-!
# The fifteen-point selected-seven-circle endpoint

The relative triple rows for the `7+8` split force the outside-pair
weight of a selected seven-circle to be at least `109`.  The elementary
two--two capacity bounds the same weight by `84`.
-/
...
/-- The four relative triple rows for the selected `7+8` split. -/
private theorem fifteenSeven_relative_rows
    (S : BlockSystem Point Block) (D : Finset Point)
    (hpoint : Fintype.card Point = 15) (hD : D.card = 7) :
    (∑ b : Block, (Nat.choose (fourteenOutside S D b) 3 : Int)) = 56 ∧
    (∑ b : Block,
      (fourteenInside S D b : Int) *
        (Nat.choose (fourteenOutside S D b) 2 : Int)) = 196 ∧
    (∑ b : Block,
      (Nat.choose (fourteenInside S D b) 2 : Int) *
        (fourteenOutside S D b : Int)) = 168 ∧
...
/-- The line residual stays nonnegative through size seven. -/
private theorem fifteenSeven_line_residual_bound (g x : Nat)
...
/-- Pointwise residual summation under the seven-block cap. -/
private theorem sum_fifteenSevenBlockFunctional_le
    ...
    (∑ b : Block,
      fourteenBlockFunctional (S.kind b)
        (fourteenInside S D b) (fourteenOutside S D b)) ≤
      4 * S.defectRow + 195 := by
  classical
  have hrow0 := (fifteenSeven_relative_rows S D hpoint hD).1
  have hlocal (b : Block) :
      fourteenBlockFunctional (S.kind b)
          (fourteenInside S D b) (fourteenOutside S D b) ≤
    ...
    · have hg : fourteenInside S D b ≤ 2 := by
      ...
      | line =>
          ...
          by_cases hmin :
              3 ≤ fourteenInside S D b + fourteenOutside S D b
          · have hres := fifteenSeven_line_residual_bound
              (fourteenInside S D b) (fourteenOutside S D b)
              hg hmin hcap
            have hdefect' :
  ...
  calc
    ...
    _ = 4 * S.defectRow + 195 := by
      simp only [Finset.sum_add_distrib, ← Finset.mul_sum]
      rw [S.defectRow_eq_sum_blockDefectContribution, hrow0]
      simp
      ring

/-- The selected `7+8` row contradicts elementary pair capacity. -/
private theorem no_selected_seven_block_at_fifteen
    (S : BlockSystem Point Block) (D : Finset Point) (gamma : Block)
    (hpoint : Fintype.card Point = 15) (hD : D.card = 7)
    (hgammaKind : S.kind gamma = .circle)
    (hgammaSupport : S.support gamma = D)
  ...
  have hexpand := sum_fifteenSevenBlockFunctional_eq S D hpoint hD
  have hupper := sum_fifteenSevenBlockFunctional_le S D gamma hpoint hD
    hgammaKind hgammaSupport hcircleCap hlineCap
  have hupper' :
      (∑ b : Block,
        fourteenBlockFunctional (S.kind b)
          (fourteenInside S D b) (fourteenOutside S D b)) ≤ 855 := by
    omega
  have hresidual :
      (1509 : Int) - 6 * (fourteenWeight S D : Int) ≤ 855 := by
    rw [hexpand] at hupper'
    omega
  have hlower : 109 ≤ fourteenWeight S D := by omega
...
/-- The `(15,7)` circle-kind finite-window residual is impossible. -/
theorem FiniteWindowRichBlockResidual.circle_impossible_of_fifteen_seven
  ...
  have hcircleCap : ∀ b, S.kind b = .circle → (S.support b).card ≤ 7 := by
    intro b hb
    exact hcapSeven b (S.circle_min b hb)
  ...
  have htotal : S.totalCircleCount ≤ 84 := by
    ...
    exact hcount84
  apply no_selected_seven_block_at_fifteen S D gamma h15 hDcard
